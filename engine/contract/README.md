...
**Bloque 3 — Precio/OHLC/técnico (Coinbase Exchange)**: el más grande de los tres, y el único con una fuente NUEVA (`engine/technical/fetch_backfill.py`, Coinbase Exchange EUR, elegida sobre Yahoo Finance por ToS/estabilidad/divisa — comparación completa en `engine/technical/README.md`). `adapt_technical_backfill()` reutiliza `score.py::historical_series()` (misma metodología de `sma`/`rsi`/`atr`/`historical_volatility` que ya usa el flujo incremental, sobre el array completo en vez de solo `[-1]`). Escribe `precio`, `volumen` (métrica nueva, también añadida a `adapt_technical()` para no divergir), `sma20/50/100/200`, `rsi14`, `atr14`, `atr14_pct_precio`, `volatilidad_hist_30d_anualizada_pct` — fuente `"Coinbase"`, nunca `"Kraken"`. Huecos reales de calendario en la serie (el mayor: XRP deslistado de Coinbase EEUU 2021-01→2023-07 por el litigio SEC-Ripple, ~905 días) se tratan como tramos independientes (`score.py::_split_contiguous()`) para que ninguna ventana móvil mezcle precios de antes y después del hueco.

**Corrección del hueco de proceso (2026-09-04, el mismo día)**: la primera versión de este bloque excluía por una frontera fija (el día antes de la primera vela cacheada de Kraken) asumiendo que esa caché reflejaba lo ya escrito en `data/metrics/` — pero el cron incremental solo escribe un punto por ejecución, nunca toda la ventana de Kraken, lo que dejó un hueco real de ~2 años detectado por el usuario en Power BI. Corregido: `adapt_technical_backfill()` ahora excluye cualquier fecha que YA esté en `data/metrics/{symbol}.json` (de cualquier fuente, vía `_existing_technical_dates()`), y `fetch_backfill.py::update_cache()` extiende la caché local hasta hoy en vez de re-descargar todo. Ver `engine/technical/README.md` para el detalle completo (fuentes evaluadas para el hueco de XRP incluidas). Resultado final (tras la corrección): BTC desde 2015-04-23 (40616 filas totales, +7650 en la corrección), ETH desde 2017-05-23 (36565, +7200), ADA desde 2021-03-18 (21238, +7200), SOL desde 2021-06-17 (20619, +7200), DOT desde 2021-06-16 (18640, +7200), XRP desde 2019-02-26 (17572, +7650) — 44100 filas nuevas en la corrección. Verificado idempotente (segunda ejecución: 0 filas nuevas en los 6 activos) y sin fechas con más de una fuente (`python3 backfill.py --gaps` para diagnóstico reutilizable).

**Bloque 4 — Precio/OHLC/técnico de acciones (Yahoo Finance)**: `adapt_technical_backfill()` generalizado con parámetros `asset_type`/`source`/`currency` (por defecto `"crypto"`/`"Coinbase"`/`"EUR"`, cero cambios de comportamiento para cripto) — acciones usan `asset_type="equity"`, `source="Yahoo Finance"`, `currency="USD"`, y por tanto el calendario de sesiones NYSE de `trading_calendar.py` en vez del calendario 24/7. Precio = `close` de Yahoo (ya ajustado por splits de forma retroactiva, verificado en vivo — nunca `adjclose`, que además ajusta por dividendos, ver `engine/technical/README.md` para el detalle completo). Mismas 10 métricas que cripto (`precio`, `volumen`, `sma20/50/100/200`, `rsi14`, `atr14`, `atr14_pct_precio`, `volatilidad_hist_30d_anualizada_pct`). Resultado real: IBM desde 1970-01-02 (136706 filas nuevas), XOM desde 1970-01-02 (136706), NVDA desde 1999-01-22, su OPV (66404) — 339816 filas en total. `detect_technical_gaps()` también generalizado con `asset_type` (antes reportaba ~1500 falsos huecos por activo en acciones, uno por cada fin de semana/festivo) — con la corrección, solo aparecen cierres reales de todo el mercado NYSE (11-S, huracán Sandy, días de luto nacional), ninguno sin explicar. Verificado idempotente y sin fechas con más de una fuente.
