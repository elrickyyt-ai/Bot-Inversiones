...
from schema import validate_metric_row, validate_thesis_row, ContractError, METRIC_FIELDS, THESIS_FIELDS
...
def run_qa():
    ...
    all_thesis_rows = [(fname, row) for fname, row in thesis_files.items()]
    ...
    for _, row in all_metric_rows:
        m, u = row.get("metric"), row.get("unit")
        if m is None:
            continue
        metric_units.setdefault(m, set()).add(u)
    for m, units in metric_units.items():
        if len(units) > 1:
            incompatible.append(f"  métrica '{m}' aparece con unidades distintas: {units}")
    ...
    p("=" * 70)
    overall = not schema_errors and not duplicates and not incompatible and not privacy_issues and not no_source
