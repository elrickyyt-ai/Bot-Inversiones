...
## Backfill histórico de acciones (2026-09-04, Bloque 4) — `fetch_backfill_equity.py`

**Fuente: Yahoo Finance** (mismo endpoint que cripto, `query1.finance.yahoo.com/v8/finance/chart/{TICKER}`, sin clave), tras verificar en vivo que Alpha Vantage gratuito bloquea `TIME_SERIES_DAILY_ADJUSTED` y `outputsize=full` ("premium endpoint"/"premium feature") -- solo da ~100 velas recientes sin ajustar por splits, insuficiente para un backfill profundo. `update_cache(symbol)` sigue el mismo patrón que el fetcher de cripto (extiende la caché local desde su última vela + 1 día hasta hoy, sin re-descargar), pero sin necesidad de paginar -- Yahoo devuelve la historia completa de un ticker en una sola petición (verificado: IBM/XOM, ~14.000 velas desde 1970, en una sola llamada).

**Tratamiento de splits -- sin código de ajuste propio.** Verificado en vivo (comparando contra el precio real conocido de NVDA antes de su split 10:1 del 2024-06-10, ~$1.208) que el campo `close` de Yahoo YA viene ajustado retroactiva y continuamente por TODOS los splits del activo (confirmado también en IBM contra sus splits de 1997/1999) -- así que no hace falta implementar ningún ajuste de split en este proyecto. **Se usa `close`, nunca `adjclose`**: `adjclose` añade ADEMÁS un ajuste por dividendos, descartado deliberadamente -- una caída de precio en el ex-dividendo es una variación real de mercado (a diferencia de un split, que es un evento puramente mecánico/cosmético) y no debe "corregirse" como si fuera un artefacto. Ventaja adicional de usar solo `close`: es estable frente a re-ejecuciones futuras del backfill (solo cambiaría si el activo hace un split nuevo, un evento raro y detectable), mientras que `adjclose` se recalcula con cada dividendo pagado (deriva continua, no una serie históricamente estable).

**Volumen**: se usa tal cual lo da Yahoo (también viene ya ajustado por splits, verificado -- sin discontinuidad en el volumen alrededor del split de NVDA). **Dividendos**: no se tratan ni se guardan -- el precio no se ajusta por ellos (ver arriba) y no existe todavía una métrica en el Data Contract para eventos de dividendo (no se ha añadido una para no ampliar el Data Contract sin una necesidad concreta).

**Trazabilidad**: `source="Yahoo Finance"` en cada fila (nunca `"Alpha Vantage"`, la fuente del flujo incremental actual de fundamentales), documentado aquí y en el docstring de `adapt_technical_backfill()`. Los splits detectados en cada descarga se imprimen (no se guardan en el Data Contract, ver arriba) -- NVDA: 6 splits (2000, 2001, 2006, 2007, 2021, y el 10:1 de 2024); IBM: 5 (incluye el ajuste de 2021-11-04 por la escisión de Kyndryl, que Yahoo representa como un "split" de ratio 1046:1000 -- tratado igual que cualquier otro split porque mecánicamente produce el mismo tipo de artefacto de continuidad si no se ajusta); XOM: 5 (todos anteriores a 2001).

**Resultado real**: IBM desde 1970-01-02 (136.706 filas nuevas), XOM desde 1970-01-02 (136.706 filas nuevas), NVDA desde 1999-01-22, su OPV (66.404 filas nuevas) -- 339.816 filas en total. `python3 backfill.py --gaps` (ahora consciente del calendario de sesiones, ver más abajo) encuentra únicamente cierres reales y documentados de todo el mercado NYSE (11 de septiembre de 2001, huracán Sandy en 2012, y varios días de luto nacional por fallecimiento de expresidentes) -- ningún hueco sin explicar. Verificado idempotente (segunda ejecución: 0 filas nuevas en los 3 activos) y sin fechas con más de una fuente.

**`backfill.py::detect_technical_gaps()` generalizado el mismo día**: antes de este bloque, ese diagnóstico usaba una comprobación de calendario natural (no consciente de sesiones) -- aplicado a una acción con décadas de historia habría reportado ~1.500 "huecos" falsos, uno por cada fin de semana/festivo. Ahora acepta `asset_type` y usa `trading_calendar.sessions_skipped_between()`, igual que `_split_contiguous()`.
